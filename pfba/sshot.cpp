...
INT32 MakeScreenShot(const char *dest)
{
	...
	png_text text_ptr[7]; //= { { 0, 0, 0, 0, 0, 0, 0 }, };
	INT32 num_text = 7;

    int nVidImageDepth = 16;
	...
	ff = fopen(dest, "wb");
	if (ff == NULL) {
		printf("MakeScreenshot: fopen failed: `%s`\n", dest);
		png_destroy_write_struct(&png_ptr, &info_ptr);
    ...
    sprintf(szDescription, "Screenshot of %s", BurnDrvGetTextA(DRV_NAME));
	...
	// png_convert_to_rfc1123_buffer random crash.. hum
	//text_ptr[4].key = "Creation Time";	png_convert_to_rfc1123_buffer(text_ptr[4].text, &png_time_now);
	text_ptr[4].key = "Software";		text_ptr[4].text = szSoftware;
	text_ptr[5].key = "Source";			text_ptr[5].text = szSource;
	text_ptr[6].key = "Comment";		text_ptr[6].text = "This screenshot was created by running the game in an emulator; it might not accurately reflect the actual hardware the game was designed to run on.";
